...
setwd("D:/workplace/Immunotherapy/TCGA/M1M2/")

load("D:/workplace/Immunotherapy/TCGA/M1M2/datasets7.Rdata")


list_train_vali_Data[[1]][is.na(list_train_vali_Data[[1]])]<-0


list_train_vali_Data[[1]]<-na.omit(list_train_vali_Data[[1]])
list_train_vali_Data[[2]]<-na.omit(list_train_vali_Data[[2]])
list_train_vali_Data[[3]]<-na.omit(list_train_vali_Data[[3]])
list_train_vali_Data[[4]]<-na.omit(list_train_vali_Data[[4]])
list_train_vali_Data[[5]]<-na.omit(list_train_vali_Data[[5]])
list_train_vali_Data[[6]]<-na.omit(list_train_vali_Data[[6]])
list_train_vali_Data[[7]]<-na.omit(list_train_vali_Data[[7]])


list_train_vali_Data[[1]]<-subset(list_train_vali_Data[[1]],
                                  list_train_vali_Data[[1]]$OS.time!=0)
list_train_vali_Data[[2]]<-subset(list_train_vali_Data[[2]],
                                  list_train_vali_Data[[2]]$OS.time!=0)

list_train_vali_Data[[3]]<-subset(list_train_vali_Data[[3]],
                                  list_train_vali_Data[[3]]$OS.time!=0)

list_train_vali_Data[[4]]<-subset(list_train_vali_Data[[4]],
                                  list_train_vali_Data[[4]]$OS.time!=0)

list_train_vali_Data[[5]]<-subset(list_train_vali_Data[[5]],
                                  list_train_vali_Data[[5]]$OS.time!=0)

list_train_vali_Data[[6]]<-subset(list_train_vali_Data[[6]],
                                  list_train_vali_Data[[6]]$OS.time!=0)

list_train_vali_Data[[7]]<-subset(list_train_vali_Data[[7]],
                                  list_train_vali_Data[[7]]$OS.time!=0)

list_train_vali_Data2<-list(Dataset1 = list_train_vali_Data[[1]], 
                            Dataset2 = list_train_vali_Data[[2]],
                            Dataset3 = list_train_vali_Data[[3]],
                            Dataset4 = list_train_vali_Data[[4]],
                            Dataset5 = list_train_vali_Data[[5]],
                            Dataset6 = list_train_vali_Data[[6]]
)
library(dplyr)
genelist<-intersect(colnames(list_train_vali_Data2[[1]])[-(1:3)],
                    colnames(list_train_vali_Data2[[2]])[-(1:3)])%>%
  intersect(colnames(list_train_vali_Data2[[3]])[-(1:3)])%>%
  intersect(colnames(list_train_vali_Data2[[4]])[-(1:3)])%>%
  intersect(colnames(list_train_vali_Data2[[5]])[-(1:3)])%>%
  intersect(colnames(list_train_vali_Data2[[6]])[-(1:3)])

###########

                       train_data = list_train_vali_Data2$Dataset1
  ...
  if(!is.na(rf_nodesize)&
    ...
    if(mode == 'all') {
      ...
      fit <- rfsrc(Surv(OS.time,OS)~., data = est_dd,
                   ...
                   seed = seed)
      rs<-list()
      for(i in 1:length(val_dd_list)){
        #if(i==1){
         # RSs  = predict(fit, newdata = as.data.frame(val_dd_list[[i]]),outcome = "train")$predicted
        #}else{
          RSs  = predict(fit, newdata = as.data.frame(val_dd_list[[i]]),outcome = "test")$predicted
        #}
        rs[[i]]<-cbind(val_dd_list[[i]][, 1:2], RS  = RSs)
      }
      ...
      if(length(rid)>1) {
        ...
        #rs<-list()
        #for(i in 1:length(val_dd_list2)){
          
         # rs[[i]]<-cbind(val_dd_list2[[i]][, 1:2], RS = as.numeric(predict(fit, type = 'response', newx = as.matrix(val_dd_list2[[i]][, -c(1, 2)]), s = fit$lambda.min)))
          #names(rs)[i]<-paste("Dataset",i,sep = "")
        #}
      ...
      if(T){
        ...
        if(length(rid)>1) {
          ...
          for(i in 1:length(val_dd_list2)){
            #if(i==1){
             # RSs  = predict(fit, newdata = as.data.frame(val_dd_list2[[i]]),outcome = "train")$predicted
            #}else{
              RSs  = predict(fit, newdata = as.data.frame(val_dd_list2[[i]]),outcome = "test")$predicted
            #}
            rs[[i]]<-cbind(val_dd_list2[[i]][, 1:2], RS  = RSs)
          }
...
write.table(genelist,"cox_dataset6_signature36.txt",row.names = F,col.names = F,quote = F)

library(Mime1)

###各模型Plot C-index：
res<-result.cindex.fit
cindex_dis_all(res,validate_set = names(list_train_vali_Data)[-1],order =names(list_train_vali_Data),width = 0.35)

saveRDS(res,"result.cindex.fit6.RData")
load("D:/workplace/Immunotherapy/TCGA/M1M2/result.cindex.fit6.RData")
###不同数据集之间特定模型的Plot C-index

res$Cindex.res$Model%>%unique()

cindex_dis_select(res,
                  model="StepCox[forward] + Enet[α=0.3]",
                  order= names(list_train_vali_Data))

####在不同数据集中，根据特定模型计算的风险评分绘制患者生存曲线

survplot <- vector("list",6) 
for (i in c(1:6)) {
  print(survplot[[i]]<-rs_sur(res, model_name = "StepCox[forward] + Enet[α=0.3]",dataset = names(list_train_vali_Data)[i],
                              #color=c("blue","green"),
                              median.line = "hv",
                              cutoff = 0.5,
                              conf.int = T,
...
auc_df<-res[["riskscore"]]["StepCox[forward] + Enet[α=0.3]"]
auc_df1<-auc_df[[1]][[1]]%>%as.data.frame()
ROC_ls<-list()

for(i in 1:length(auc_df[[1]])){
  
  x<-auc_df[[1]][[i]]%>%as.data.frame()
