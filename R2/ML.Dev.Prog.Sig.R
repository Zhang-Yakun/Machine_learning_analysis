...
#load("D:/workplace/Immunotherapy/mime_survival/test/Example.cohort.Rdata")
#load("D:/workplace/Immunotherapy/mime_survival/test/genelist.Rdata")


####
load("D:/workplace/Immunotherapy/mime_survival/datasets8.Rdata")
list_train_vali_Data <- list(Dataset1 = list_train_vali_Data[["Dataset1"]], 
                             Dataset2 = list_train_vali_Data[["Dataset2"]],
                             Dataset3 = list_train_vali_Data[["Dataset3"]], 
                             Dataset4 = list_train_vali_Data[["Dataset5"]], 
                             Dataset5 = list_train_vali_Data[["Dataset6"]], 
                             Dataset6 = list_train_vali_Data[["Dataset7"]], 
                             Dataset7 = list_train_vali_Data[["Dataset8"]])
####
library(dplyr)
genelist<-intersect(colnames(list_train_vali_Data[[1]])[-(1:3)],
                    colnames(list_train_vali_Data[[2]])[-(1:3)])%>%
  intersect(colnames(list_train_vali_Data[[3]])[-(1:3)])%>%
  intersect(colnames(list_train_vali_Data[[4]])[-(1:3)])%>%
  intersect(colnames(list_train_vali_Data[[5]])[-(1:3)])%>%
  intersect(colnames(list_train_vali_Data[[6]])[-(1:3)])%>%
  intersect(colnames(list_train_vali_Data[[7]])[-(1:3)])
...
res <- ML.Dev.Prog.Sig(train_data = list_train_vali_Data$Dataset1,
                       ...
                       mode = 'all',nodesize =5,seed = 123456 )
...
saveRDS(res,"result.cindex.fit7.RData")

###不同数据集之间特定模型的Plot C-index

cindex_dis_select(res,
                  model="StepCox[forward] + Ridge",
                  order= names(list_train_vali_Data))

####在不同数据集中，根据特定模型计算的风险评分绘制患者生存曲线

survplot <- vector("list",7) 
for (i in c(1:7)) {
  print(survplot[[i]]<-rs_sur(res, model_name = "StepCox[forward] + Ridge",dataset = names(list_train_vali_Data)[i],
                              #color=c("blue","green"),
                              median.line = "hv",
                              cutoff = 0.5,
                              conf.int = T,
